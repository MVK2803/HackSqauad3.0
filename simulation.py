...
signals = []
ns = 4
stime = 300       #time of simulation
timeElapsed = 0

currentGreen = 0   
nextGreen = (currentGreen+1)%ns
currentYellow = 0   
...
def initialize():
    ts1 = TrafficSignal(0, dy, dg, dmin, dmax)
    signals.append(ts1)
    ts2 = TrafficSignal(ts1.red+ts1.yellow+ts1.green, dy, dg, dmin, dmax)
    signals.append(ts2)
    ts3 = TrafficSignal(dr, dy, dg, dmin, dmax)
    signals.append(ts3)
    ts4 = TrafficSignal(dr, dy, dg, dmin, dmax)
    signals.append(ts4)
    repeat()

# Set time according to formula
def setTime():
    global nc, nb, nbus, nt, nr, nl
    global carTime, busTime, truckTime, rickshawTime, bikeTime
    nc, nbus, nt, nr, nb = 0,0,0,0,0
    ...
    greenTime = math.ceil(((nc*carTime) + (nr*rickshawTime) + (nbus*busTime) + (nt*truckTime)+ (nb*bikeTime))/(nl+1))
    print('Green Time: ',greenTime)
    if(greenTime<dmin):
        greenTime = dmin
    elif(greenTime>dmax):
        greenTime = dmax
    signals[(currentGreen+1)%(ns)].green = greenTime
...
def repeat():
    ...
    while(signals[currentGreen].green>0):   
        printStatus()
        updateValues()
        if(signals[(currentGreen+1)%(ns)].red==detectionTime):    
    ...
    while(signals[currentGreen].yellow>0):  
        ...
        time.sleep(1)
    currentYellow = 0   
    signals[currentGreen].green = dg
    signals[currentGreen].yellow = dy
    signals[currentGreen].red = dr
...
def simulationTime():
    global timeElapsed, stime
    while(True):
        timeElapsed += 1
        time.sleep(1)
        if(timeElapsed==stime):
            totalVehicles = 0
